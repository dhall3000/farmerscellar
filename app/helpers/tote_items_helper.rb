module ToteItemsHelper
  ...
  def friendly_date_text(datetime)
    ...
    if datetime.midnight == Time.zone.now.midnight
      delivery_date_text = "today"
    elsif datetime.midnight == Time.zone.now.midnight - 1.day
  ...
  def get_authorized_subscriptions_for(user)
    ...
    return Subscription.joins(rtauthorizations: :rtba).where(user: user, on: true, kind: Subscription.kinds[:NORMAL]).where("rtauthorizations.rtba_id" => rtba.id).where("rtbas.active" => true).distinct

  end

  def get_authorized_subscription_objects_for(user)
    ...
    return Subscription.joins(rtauthorizations: :rtba).where(user: user, on: true).where("rtauthorizations.rtba_id" => rtba.id).distinct
