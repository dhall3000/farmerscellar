...
class IntegrationHelper < ActionDispatch::IntegrationTest
  ...
  def verify_header(tote = 0, orders = 0, calendar = 0, subscriptions = 0, ready_to_pickup = 0, whats_new = nil)
...
if false
    ...
    if all_items_fully_filled?(tote_items)
      ...
      #we're no longer displaying zero-filled items so verify there aren't any of those
      assert_equal 0, tote_items.where(quantity_filled: 0).count
  ...
  def create_tote_item(customer, posting, quantity, frequency = nil, roll_until_filled = nil)
    ...
    if posting.biggest_order_minimum_producer_net_outstanding > 0      
      assert_select 'div.row.gutter-10 div.col-xs-12 div.alert.alert-danger span', {text: "Unmet Club Order Minimum. Current shortfall is #{ActiveSupport::NumberHelper.number_to_currency(posting.reload.biggest_order_minimum_producer_net_outstanding)}", count: 1}
    end
