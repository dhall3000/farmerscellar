...
class ProducerNotificationsMailerPreview < ActionMailer::Preview
  ...
  def current_orders_product_id_code
    ...
    ProducerProductUnitCommission.create(user: farmer, product: Product.first, unit: Unit.first, commission: 0.05)
    posting1 = Posting.new(unit: Unit.first, product: Product.first, user: farmer, description_body: "descrip", price: 1.25, live: true, order_cutoff: Time.zone.now - 1.second, delivery_date: delivery_date, product_id_code: "awesomxyz")
    posting1.save
    #make some tote items for that posting
    user = User.find_by(email: "c1@c.com")    
    ...
    posting2 = Posting.new(unit: Unit.first, product: Product.first, user: farmer, description_body: "descrip", price: 1.25, live: true, order_cutoff: Time.zone.now - 1.second, delivery_date: delivery_date)
    posting2.save
    #make some tote items for that posting
    user = User.find_by(email: "c1@c.com")    
  ...
  def current_orders_fancy
    ...
    posting3 = Posting.new(unit: Unit.first, product: Product.first, user: farmer, description_body: "descrip", price: 1.25, live: true, order_cutoff: Time.zone.now - 1.second, delivery_date: delivery_date, units_per_case: 2)
    posting3.save
    #make some tote items for that posting
    user = User.find_by(email: "c1@c.com")    
